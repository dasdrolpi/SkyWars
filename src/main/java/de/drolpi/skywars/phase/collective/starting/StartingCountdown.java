...
public class StartingCountdown extends Countdown {
    ...
    public StartingCountdown(SkyWars skyWars, CounterInformation information) {
        ...
        switch (counter.getCurrentTime()) {
            case 5: case 4: case 3: case 2: case 1:
                for (Player player : server.getOnlinePlayers()) {
                    player.sendTitle(messageProvider.getString(Locale.GERMAN, "skywars.starting.countdown_title_" + counter.getCurrentTime(), counter.getCurrentTime()), "");
    ...
    public void onFinish(Counter counter) {
        for (Player player : server.getOnlinePlayers()) {
            player.sendTitle(messageProvider.getString(Locale.GERMAN, "skywars.starting.countdown_title_finish"), "");
