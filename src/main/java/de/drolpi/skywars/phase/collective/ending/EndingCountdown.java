...
public class EndingCountdown extends Countdown {
    ...
    public EndingCountdown(SkyWars skyWars, CounterInformation information) {
                ...
                for (Player player : server.getOnlinePlayers()) {
                    player.sendMessage(
                            messageProvider.getString(Locale.GERMAN, "skywars.prefix")
                                    + messageProvider.getString(Locale.GERMAN, "skywars.ending.countdown_tick", counter.getCurrentTime())
                    );
    ...
    public void onFinish(Counter counter) {
        for (Player player : server.getOnlinePlayers()) {
            player.sendMessage(
                    messageProvider.getString(Locale.GERMAN, "skywars.prefix")
                            + messageProvider.getString(Locale.GERMAN, "skywars.ending.countdown_finish")
            );
