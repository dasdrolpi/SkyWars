...
public class InGameCountdown extends Countdown {
    ...
    public InGameCountdown(SkyWars skyWars, CounterInformation information) {
        super(information.getDuration(), 0, 1, TimeUnit.MINUTES);
                ...
                for (Player player : server.getOnlinePlayers()) {
                    player.sendMessage(
                            messageProvider.getString(Locale.GERMAN, "skywars.prefix")
                                    + messageProvider.getString(Locale.GERMAN, "skywars.ingame.countdown_tick", counter.getCurrentTime())
                    );
